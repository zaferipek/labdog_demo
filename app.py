"""
LabDog — Ar-Ge Yönetim Sistemi
Entry point: auth gate + dashboard shell (sidebar + routing).
Page content lives in views/.

─── Session persistence ───────────────────────────────────────
Login  → create_session() → opaque token → ?token= in URL
Reload → main() reads ?token= → restore AuthUser from session store
Logout → destroy_session() + st.query_params.clear()

─── Browser history (Back / Forward) ──────────────────────────
Streamlit uses history.replaceState() internally, so every page
update *replaces* the current history entry — the Back button never
sees a previous page.

We inject a one-shot JS shim that intercepts replaceState():
  • When ``page`` *or* any tracked detail param changes (``project_id``,
    ``project_tab``, ``formulation_id``, ``task_id``, ``material_id``, …)
    → use pushState() instead (real history entry for Geri/İleri).
  • Same page *and* same detail fingerprint → let replaceState() pass
    through (widget noise, sidebar collapse).

popstate → location.reload() (Streamlit cannot align client URL without
full reload). ``?token=`` + query params restore state after reload.
"""
...
# Ana menü (sidebar radyo). Ayarlar ayrı sayfa; ⚙ ile açılır, menüde yok.
...
def _logo_bytes() -> bytes:
    """Read logo PNG once and keep it in memory for the server lifetime."""
    with open(_LOGO, "rb") as fh:
        return fh.read()


def _login_logo_html(width: int = 200) -> str:
    """Giriş ekranı: st.image kullanmadan logo (tam ekran butonu oluşmaz)."""
...
# ──────────────────────────────────────────────────────────────
# BROWSER HISTORY MANAGER  (Back / Forward support)
# ──────────────────────────────────────────────────────────────
...
def _inject_history_manager() -> None:
    ...
    • ``replaceState`` / ``pushState`` — sayfa veya detay query fingerprint değişince geri yığını
    • ``popstate`` — ``location.reload()``: Streamlit, programatik tıklamada adres çubuğu
      URL'sini sunucu ``query_params`` ile güvenilir eşleştirmediği için tam yenileme gerekir.
    • ``pageshow`` (bfcache): WebSocket tutarlılığı için hâlâ ``reload``.
    """
    components.html(
        """
        <script>
...
def _sidebar_fragment() -> None:
    """
    Sidebar ``@st.fragment`` içinde: dar/genişlet ve form etkileşimleri yalnızca
    bu bölümü yeniler.

    Sayfa menüsü / Ayarlar / Çıkış → ``st.rerun(scope=\"app\")`` (sağ sütun da
    güncellenir). Sağ taraftaki sayfa gövdesi ayrıca ``_main_content_fragment``
    ile sarıldığı için çoğu ana-alan tıklaması sol menüyü yeniden çizmez.
    """
    ...
    # Dar mod: mutlak konumlu toggle akışta yer tutmaz; nav ikonlarını aşağı al
    if collapsed:
        st.markdown(
    ...
    # Geniş sidebar: gerçek logo + marka; dar modda DOM'a hiç sokma
    if not collapsed:
    ...
    # Navigation change → content must update → full app rerun
    ...
    # Hidden CSS marker for :has(.ld-sb-collapsed) width rules
    if collapsed:
        st.markdown(
...
def _apply_user_startup_preferences(user) -> None:
    """Kullanıcı tercihlerine göre ilk URL / nav / dashboard alt görünüm."""
...
def _maybe_bootstrap_startup_page() -> None:
    """
    İlk oturum yüklemesinde ?page= yoksa veya geçersizse kullanıcı tercihlerini uygula.
    Yer imi / geçmişte geçerli ?page= varsa dokunulmaz.
    """
...
def _apply_search_navigation() -> None:
    """Arama sonucu tıklanınca sayfa, query_params ve proje/görev odağı."""
...
def _sync_page_from_url() -> None:
    """
    ?page= ile oturum senkronu. Ayarlar ana menüde yok; bu sayfadayken URL ezilmez.
    """
    if "last_main_nav_index" not in st.session_state:
        st.session_state["last_main_nav_index"] = 0
...
def _dashboard_screen() -> None:
    """
    Oturum kabuğu — URL ile oturum sırası (çakışma önleme):

    1. ``_apply_search_navigation`` — yalnız arama sonucu tıklanınca; ``query_params`` yazar.
    2. ``_maybe_bootstrap_startup_page`` — yalnız ilk yüklemede ve geçerli ``?page=`` yokken.
    3. ``_sync_page_from_url`` — adres çubuğu ↔ ``nav_index``; ana gerçeklik kaynağı burada
       (Ayarlar sayfası iken ``page=`` ezilmez).
    """
    st.markdown(f"<style>{base_css()}{dashboard_css()}</style>", unsafe_allow_html=True)

    _inject_history_manager()   # one-shot JS shim — idempotent
    _apply_search_navigation()  # arama → sayfa / detay query + session odağı
    _maybe_bootstrap_startup_page()  # tercihlere göre ilk sayfa (page yoksa)
    _sync_page_from_url()       # URL ↔ sidebar nav — rerun yok
...
def _main_content_fragment() -> None:
    ...
    user  = st.session_state["user"]          # AuthUser — needed for Ayarlar / role checks
    stats = get_dashboard_stats()             # served from @st.cache_data — zero DB cost
...
# ──────────────────────────────────────────────────────────────
# MAIN — session restore + auth gate
# ──────────────────────────────────────────────────────────────

def main() -> None:
    # Restore session from URL token before anything renders (handles F5 / reload)
